...
router.post('/', (_req: Request, res: Response) => {
  ...
  const result = db.prepare('INSERT INTO teams (id, name, color, members) VALUES (?, ?, ?, ?)').run(id, name, color || '#1890ff', membersJson);
...
router.put('/:id', (_req: Request, res: Response) => {
  ...
  const result = db.prepare(
    'UPDATE teams SET name = COALESCE(?, name), color = COALESCE(?, color), members = COALESCE(?, members) WHERE id = ?'
  ).run(name, color, membersJson, _req.params.id);
...
router.delete('/:id', (_req: Request, res: Response) => {
  const db = getDb();
  const result = db.prepare('DELETE FROM teams WHERE id = ?').run(_req.params.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Team not found' });
